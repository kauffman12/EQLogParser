...
2. EQLP now includes a **New Feature** for selecting an optional **TTS Engine**. Code provided by **willk@github**. Look under the **Tools** dropdown on **Trigger Manager** to try out/configure the **TTS Engine**. The additional choices are for **Kokoro** and **PiperTTS**. Brief descritpions are povided within the app.
3. **Linux Users** you will have to install a TTS Engine. The old default will no longer work.
...
3. **NAG Migration** as requested, if you want to attempt an import of your NAG database to EQLP there is a way to do this now under the **Tools** menu. It does not handle everything but it should cover a good amount. Note that it will generate a report to explain what worked and what didn't. To try it, click **Migrate NAG Database** and choose the NAG folder with all the json files.
...
3. Updated **Timer Overlays** sorting options to include **Timer Name (Natural)**. This will allow numbers in the **Timer Name** to be sorted numerically.
4. Added **Show Milliseconds** option to **Timer Overlays**.
5. Fixed order of **Timer Overlay** options.
6. Included latest Syncfusion and replaced some old libraries.
...
4. Many more **Trigger Processing** improvements.
5. Hopefully a fix for EMU crit parsing.
