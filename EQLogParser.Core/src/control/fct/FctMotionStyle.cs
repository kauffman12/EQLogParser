namespace EQLogParser
{
  ...
  internal enum FctMotionStyle
  {
    ...
    /*
     * Constant vertical speed up the owning lane plus a quadratic outward drift — the parabola, which is the shape the
     ...
     * ingest degrades it to hold there. Its train is kept in step by the conveyor (FctConveyor), which owns its spacing.
     */
    Parabola,

    /*
     * The split shape with the arc taken out: the same rail as the parabola — constant scroll, shared entrance, one clock
     * per column and all — running with bow zero, so each number climbs or sinks its lane in a straight line. MSBT calls it
     * Straight and ships it as the no-nonsense alternative to Parabola; the reason it is
