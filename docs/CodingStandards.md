...
- **Row shape.** One `Grid` per section: fixed label column (106) + `*` control column, labels one short lowercase phrase (`thin bars`, `class filter`); checkboxes span both columns.
- **Wording.** Accordion headers are caps (`STYLE`); the window title bar text and header strip are sentence case (`Damage Meter Setup`). Tooltips describe what the player gets, not what the flag is called, and are written as a Capital-start phrase with no trailing period (`Folder where Log File Archives are kept`) — the shape every tooltip in the app has had since before the settings windows existed.
- **Tooltips are the exception.** A label that names the control needs no tooltip: `speed`, `damage in`, `crits` already say it, and a tooltip repeating them in a longer sentence is noise the player reads once and then ignores. Write one only where the row hides something a label cannot (`threshold`: small numbers vanish), and say that one thing in a plain sentence — no history, no rationale for the shipped value, no caveats about effects the player never has to reason about.
