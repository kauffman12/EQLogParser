...
namespace EQLogParser.Wpf.Test
{
  ...
  public class UiThreadProbeTest
  {
    ...
    public void AThreadHeldBySomethingElseReadsAsBlocked()
    {
      ...
      StringAssert.Contains(observed, "blocked", $"a thread waiting on an event should read as blocked (actual: {observed})");
      StringAssert.Contains(observed, "2/2 samples waiting", $"both samples were taken while it waited (actual: {observed})");

      // The reason is the useful half of the line — it is what points at a lock rather than at the render thread.
      StringAssert.Contains(observed, "(", $"a wait should name what it waited on (actual: {observed})");
    }
    ...
    public void AThreadDoingWorkReadsAsRunning()
    {
      ...
      StringAssert.Contains(observedMidFlight, "running", $"a spinning thread should read as running (actual: {observedMidFlight})");
      Assert.IsFalse(observedMidFlight.Contains("blocked"), $"a spinning thread is not blocked (actual: {observedMidFlight})");

      // CPU time on the line is what makes the count checkable: two samples of a spin are tens of milliseconds of it.
      StringAssert.Contains(observedMidFlight, "cpu", $"the line should carry the cpu the thread used (actual: {observedMidFlight})");
    }

    [TestMethod]
    public void AThreadThatCannotBeWatchedSaysSo()
    {
      // An id that does not exist, and the zero a failed capture leaves behind, must report the absence rather than guess.
      UiThreadProbe.BeginEpisode(999_999);
      UiThreadProbe.SampleEpisode(999_999);

      var observed = UiThreadProbe.EndEpisode();
      Assert.AreEqual("ui thread: n/a", observed, "a thread the probe cannot find should report nothing rather than a verdict");

      UiThreadProbe.BeginEpisode(0);
      Assert.AreEqual("ui thread: n/a", UiThreadProbe.EndEpisode(), "an uncaptured thread id should not read as healthy");
