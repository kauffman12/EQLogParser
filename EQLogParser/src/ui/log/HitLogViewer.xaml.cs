  ...
  public partial class HitLogViewer : UserControl, IDisposable
  {
    ...
    public HitLogViewer()
    {
      ...
      AddColumn(new GridTextColumn
      {
        ...
        TextAlignment = TextAlignment.Center,
        HeaderText = "Since",
        Width = ThemeConfig.CurrentShortWidth
      });
      ...
      _columnIds.ForEach(name =>
      {
        ...
        column.Width = name != "Strikethrough" ? ThemeConfig.CurrentShortWidth : DataGridUtil.CalculateMinGridHeaderWidth(name);
      ...
      AddColumn(new GridTextColumn
      {
        MappingName = "Actor",
        HeaderText = "",
        Width = ThemeConfig.CurrentNpcWidth
      });
      ...
      double currentTime, bool defending = false)
    {
      ...
      if (action is DamageRecord damage)
      {
        if (!defending && !string.IsNullOrEmpty(damage.Attacker) && !string.IsNullOrEmpty(playerStats.OrigName) && StatsUtil.IsHitType(damage.Type))
        {
          ...
          (isPet = playerStats.OrigName.Equals(PlayerRegistry.Instance.GetPlayerFromPet(damage.Attacker), StringComparison.OrdinalIgnoreCase) ||
